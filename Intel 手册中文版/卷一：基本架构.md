...
__第十二章 - 使用 Intel 流式 SIMD 三代扩展（Intel SSR3），补充 Intel 流式SIMD三代扩展（Intel SSSR3），Intel 流式 SIMD 四代扩展（Intel SSR4）和 Intel AES 新指令集（Intel AES-NI）编程。__ 列出所有 SSE3，补充 SSE3，SSE4，AESNI 指令集以及书写访问这些扩展的代码准则。         
